...
\vskip .3cm
Preuve. --- Immédiat à partir des assertions analogues pour les objets de cohomologie (SGA5 XV {\bf 3.1}).
\vskip .3cm
{
...
\vskip .3cm
Preuve. --- On peut supposer que $E$ est un $\mathbf{Q}_\ell$-faisceau constructible, et alors l'assertion n'est autre que (SGA5 XV {\bf 3.2}).
\vskip .3cm
{
...
\vskip .3cm
Nous allons maintenant déduire de ({\bf 3.3}) une \emph{équation fonctionnelle} pou les fonctions $L$, du moins si $X$ est projectif sur $\mathbf{F}_p$.
\vskip .3cm
{
...
\vskip .3cm
Preuve. --- Le second membre a un sens d'après ({\bf 3.3}). Posons $S = \Spec \mathbf{F}_p$ et $\D_S = \bRd {\cHom}_{\mathbf{Q}_\ell}(. , \mathbf{Q}_\ell)$. D'après ({\bf 2.3.2} a)), on a 
$$
...
$$
Comme $L_E = L_{\bRd g_* (E)}$ ({\bf 3.3}), l'assertion résultera du lemme suivant
\vskip .3cm
{
...
\vskip .3cm
D'après les propriétés d'additivité et de multiplicativité ({\bf 3.5.1}) et ({\bf 3.5.2}), on peut supposer que $F \in \mathbf{Q}_\ell-\fscn (S)$. Alors $F$ correspond (SGA5 VII {\bf 1.4.2}) à un $\mathbf{Q}_\ell$-espace vectoriel de dimension finie $V$ muni d'une opération continue $f_V$ du Frobenius, et le $\mathbf{Q}_\ell$-faisceau $\D_S (F) = {\cHom}_{\mathbf{Q}_\ell}(F, \mathbf{Q}_\ell)$ correspond (II {\bf 1.26}) au $\mathbf{Q}_\ell$-espace vectoriel $V^{\vee}$ muni de l'opération continue $(f^{\vee}_V)^{-1}$ du Frobenius. Il suffit alors de montrer que, étant donnés un corps $K$, un $K$-espace vectoriel de dimension finie $V$ et un automorphisme $u$ de $V$, on a l'identité
$$
1/\det(1 - u^{-1} t) = (-t)^{-\text{dim}(V)} \det(u)/\det(1 - ut^{-1})
\leqno{(3.8)}
$$
dans $K(t)$. On peut pour cela supposer $K$ algébriquement clos, donc $u$ triangulable, puis, grâce aux propriétés de multiplicativité du déterminant, que dim$(V) = 1$. Alors $u$ est l'homothétie définie par un scalaire non nul $\lambda$, et ({\bf 3.8}) est l'identité évidente
$$
1/(1-(t/\lambda)) = (-\lambda/t)/(1-(\lambda/t)).
$$
Bien entendu, la formule ({\bf 3.6}) ne présente d'intérêt en pratique que si l'on dispose d'une expression simple pour $\D_X(E)$. Nous allons maintenant donner des cas où il en est ainsi.
\vskip .3cm
{
...
\vskip .3cm
Preuve. --- Comme $\D_X(E) = \bRd {{\cHom}}_{\mathbf{Q}_\ell}(E, \mathbf{Q}_\ell(n))[2n]$, le cas $(i)$ résulte du lemme suivant.
\vskip .3cm
{
...
\vskip .3cm
Il s'agit de voir que si $F \in \mathbf{Z}_\ell-\fsct(X)$ et $G \in \mathbf{Z}_\ell-\fscn(X)$, les $\mathbf{Z}_\ell$-faisceaux $\cExt^j_{\mathbf{Z}_\ell}(F, G)$ $(j \geq 1)$ sont annulés par une puissance de $\ell$. D'après (I {\bf 6.4.2}) et (II {\bf 1.2.1}), on peut, quitte à se restreindre à des parties localement fermées convenables de $X$, supposer que $G \in \mathbf{Z}_\ell-\fsct(X)$. Alors, compte tenu de (II {\bf 1.26}), l'assertion résulte de l'assertion analogue, bien connue, pour les $\mathbf{Z}_\ell$--Modules de type fini. Montrons $(ii)$. 

Il s'agit de voir que
$$
P^j = {\cExt}^j_{\mathbf{Q}_\ell}(E, \mathbf{Q}_\ell(1)) = 0 \quad (j \geq 1).
$$
Comme $M$ est constante tordu constructible, il résulte du cas $(i)$ que $P^j | U = 0$. Il nous suffit donc de voir que pour tout point fermé $x$ de $Y = X - U$ et tout point géométrique $\overline{x}$ au-dessus de $x$, on a $P^j_x = 0$. Le pendant pour les $\mathbf{Q}_\ell$-faisceaux de la variante (SGA5 I {\bf 4.6.2}) du théorème de dualité locale fournit un accouplement parfait
$$
{\cExt}^j_{\mathbf{Q}_\ell}(E, \mathbf{Q}_\ell(1)) \times \bH^{2-j}_{\overline{x}}(E) \to \mathbf{Q}_{\ell},
\leqno{(3.9.2)}
$$
avec (SGA5 I {\bf 4.5.1})
$$
...
E \to i_* i^* (E)
$$
est un isomorphisme, il résulte de la première suite exacte de (SGA4 V {\bf 4.5}) que 
$$
\bH^0_x(E) = \bH^1_x(E) = 0,
$$
d'où aussitôt le résultat annoncé.

Ceci dit, lorsque $X$ est projectif sur $\mathbf{F}_\ell$, la formule ({\bf 3.6}) prend la forme
$$
L_{E^\vee}(p^{-n} t) = (-1)^{- \chi (E)} \delta(E) L_E (t^{-1}),
\leqno{(3.10)}
$$
dans chacun des cas de ({\bf 3.9}). Compte tenu de ({\bf 3.2} a)), cela résulte du lemme suivant.
\vskip .3cm
{
...
\vskip .3cm
D'après les propriétés de multiplicativité ({\bf 3.2} a)), on peut pour le voir supposer que $F$ est un $\mathbf{Q}_\ell$-faisceau constructible; alors, comme le Frobenius opère sur $\mathbf{Q}_\ell(j) \isom \mathbf{Q}_\ell$ (non canoniquement) par l'homothétie de rapport $p^{-j}$, l'assertion est immédiate sur la définition ({\bf 3.0}).

Supposons maintenant qu'on ait de plus un isomorphisme
$$
E^\vee \isomlong E(\rho) \quad \text{pour un}~\rho \in \mathbf{Z}.
$$
Alors la formule ({\bf 3.10}) prend la forme
$$
...
$$
\vskip .3cm
Remarque {\bf 3.13}. Sous les hypothèses de ({\bf 3.9}), l'existence d'un tel entier $p$ est assurée dans les cas suivants
$$
\begin{aligned}
    \text{cas }(i)  &\quad E \isomlong \mathbf{Q}_\ell(m) \quad \text{pour un } m \in \mathbf{Z}, \text{ et alors } \rho = -2m, \\
    \text{cas }(ii) &\quad M \isomlong \mathbf{Q}_\ell(m) \quad \text{pour un } m \in \mathbf{Z}, \text{ et alors } \rho = -2m.
\end{aligned}
$$
...
\vskip .3cm
Preuve. --- La substitution $t \mapsto 1/qt$ dans ({\bf 3.12}) fournit l'équation fonctionnelle
$$
L_E(t) = (-1/qt)^{\chi(E)}\delta(E)L_E(1/qt).
\leqno{(3.12~\text{bis})}
$$
Multipliant ({\bf 3.12}) et ({\bf 3.12} bis) membre à membre, on obtient l'identité 
$$
L_E(t)L_E(1/qt) = q^{-\chi(E)}(\delta(E))^2 L_E(t) L_E(1/qt),
$$
d'où aussitôt la relation désirée, compte tenu du fait que $L_E$ n'est pas identiquement nulle, comme il est clair sur sa définition ({\bf 3.0}).
