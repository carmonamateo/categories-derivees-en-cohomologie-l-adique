...
Lorsque $f$ est une immersion fermée, on dispose d'un tel morphisme, à savoir le morphisme d'adjonction déduit de ({\bf I 7.7.12} $(i)$).
...
\leqno{(2.1.2)}
$$
Comme $A \boldsymbol{\otimes}_{\mathbf{Z}_\ell}\mathbf{Z}_\ell(r)$ est localement libre constructible, on définit sur le modèle de ({\bf 1.1.7}), mais sans hypothèse de degré sur $F$, un isomorphisme de ``projection''
$$
...
$$
ce qui ramène à faire la construction de ({\bf 2.1.2}) dans le cas où $A = \mathbf{Z}_\ell = F$. Dans ce cas, comme $\Rd^i f_! = 0$ pour $i > 2d$ ({\bf 1.1.8}), il s'agit d'exhiber un morphisme ``trace''
$$
...
$$
Dans le cas général, on choisit pour définir ({\bf 2.1.1}) une factorisation $f = p \circ i$ du type ({\bf 1.2.2}). Désignant par
\[\begin{tikzcd}
	{u: \bRd i_! \bRd i^!} & \id \\
	{v: \bRd p_! \bRd p^!} & \id
...
\leqno{(2.2.2)}
$$
On prendra alors pour ({\bf 2.2.1}) le morphisme composé
\[\begin{tikzcd}
...
\end{tikzcd}\]
Il reste à définir ({\bf 2.2.2}). Lorsque $f$ est une immersion ouverte, le foncteur $f_!$ commute aux limites inductives filtrantes, et permet donc de définir pour tout couple $(E, F)$ de $A$-faisceaux sur $X$ un morphisme fonctoriel
$$
f_! {\cHom}_A (E, F) \to {\cHom}_A (f_! E, f_! F),
\leqno{(2.2.3)}
$$
à partir des morphismes analogues dans la catégorie des $A$--Modules. Pour définir ({\bf 2.2.2}) dans ce cas, on peut supposer $L$ quasilibre et $M$ flasque, de sorte que ${\cHom}^{\bullet}_A(L, M)$ est flasque. Le morphisme ({\bf 2.2.3}) fournit par fonctorialité un morphisme de complexes
$$
f_! {\cHom}^{\bullet}_A (L, M) \to {\cHom}^{\bullet}_A (f_! L, f_! M).
\leqno{(2.2.4)}
$$
Choisissant une résolution quasilibre $P \to f_! L$ et une résolution flasque $f_! M \to Q$, on prend pour ({\bf 2.2.2}) le composé de ({\bf 2.2.4}) et du morphisme canonique
$$
...
\bRd f_* \bRd {\cHom}_A (L, M) \to \bRd {\cHom}_A(\bRd f_* L, \bRd f_* M).
$$
La construction que nous allons faire de ({\bf 2.2.5}) vaut plus généralement pour un morphisme quasicompact et quasiséparé. Cette dernière hypothèse implique que le foncteur $f_*$ commute aux limites inductives filtrantes, et permet donc comme précédemment de définir pour tout couple $(E, F)$ de $A$-faisceaux sur $X$ un morphisme fonctoriel 
$$
f_* {\cHom}_A (E, F) \to {\cHom}_A (f_* E, f_* F),
\leqno{(2.2.6)}
$$
à partir des morphismes analogues dans la catégorie des $A$--Modules. Pour définir ({\bf 2.2.5}), on peut supposer $L$ quasilibre et $M$ flasque, de sorte que ${\cHom}^{\bullet}_A(L, M)$ est flasque. Le morphisme ({\bf 2.2.6}) fournit par fonctorialité un morphisme de complexes
$$
f_* {\cHom}^{\bullet}_A (L, M) \to {\cHom}^{\bullet}_A (f_* L, f_* M).
\leqno{(2.2.7)}
$$
On prend pour ({\bf 2.2.5}) le composé de ({\bf 2.2.7}) et du morphisme
$$
{\cHom}^{\bullet}_A (f_* L, f_* M) \to {\cHom}^{\bullet}_A (P, f_* M)
$$
déduit d'une résolution quasilibre $P \to f_* L$ de $f^* L$.

Enfin, dans le cas général, on choisit une compactification $f = q \circ i$ de $f$, et on définit ({\bf 2.2.2}) de fa\c{c}on évidente à partir des morphismes déjà définis pour $i$ et $q$ respectivement. Bien, entendu, on s'assure que le résultat ne dépend pas des choix faits, et notamment de la compactification choisie.
\vskip .3cm
{
...
\vskip .3cm
Preuve. --- Nous allons voir que ({\bf 2.2.1}) est un isomorphisme. Les autres assertions en résulteront en appliquant aux deux membres les foncteurs respectifs $\bRd \overline{\Gamma}(Y, .)$, $\bRd \Gamma(Y, .)$ (I {\bf 7.4.10}) et $\Hom_A(A, .)$ d'après (I {\bf 7.4.18}). Pour voir que ({\bf 2.2.1}) est un isomorphisme, on se ramène par le way-out functor lemma au cas où $E$ et $F$ sont les complexes de degré 0 associés à des $A$-faisceaux notés de même. Les constructions aboutissant à la définition de ({\bf 2.2.1}) peuvent alors être faites au moyen de résolutions flasques ou quasilibres dans $\mathcal{E}(X, J)$ et $\mathcal{E}(Y, J)$. Si $E = (E_n)_{n \in \mathbf{N}}$ et $F = (F_n)_{n \in \mathbf{N}}$, l'assertion résulte alors aussitôt du fait que les morphismes de dualité
$$
...
\vskip .3cm
Explicitons ({\bf 2.3.2}). --- Comme $\D_K(F) \in \D^b_c(X, \mathbf{Z}_\ell)$, le morphisme (I {\bf 7.6.5})
$$
\Hom_{\mathbf{Z}_\ell}(\D_K (F) \boldsymbol{\otimes} F, K) \to \Hom_{\mathbf{Z}_\ell}(\D_K(F), \D_K(F))
$$
est un isomorphisme (II {\bf 2.6}). L'image inverse de l'identité de $\D_K(F)$ définit un morphisme
$$
...
\vskip .3cm
{
Proposition {\bf 2.3.2} (Formules d'échange). --- \it Soient $X$ et $Y$ deux schémas noethériens de caractéristiques résiduelles premières à $\ell$, et $f: X \to Y$ un morphisme quasiprojectif. On suppose que $Y$ admet un Module inversible ample. Étant donné $K_Y \in \D^+(Y, \mathbf{Z}_\ell)$, on pose
$$
...
\vskip .3cm
Preuve. --- Formellement identique à celle de (SGA5 I {\bf 1.12}), dont d'ailleurs ({\bf 2.3.2}) n'est qu'une paraphrase.
\vskip .3cm
{
Proposition {\bf 2.3.3}. --- \it Soit $X$ un schéma noethérien de caractéristiques résiduelles premières à $\ell$.
\begin{itemize}
    \item[(i)] Si $X$ est régulier, de dimension finie, et satisfait aux conditions de (SGA5 I {\bf 3.4.1}), le complexe $\mathbf{Z}_\ell$ est dualisant sur $X$.
...
\vskip .3cm
Preuve. --- Montrons par exemple $(ii)$, les autres assertions se prouvant de fa\c{c}on essentiellement identique, à partir des énoncés correspondants de (SGA5 I). Montrons tout d'abord que si $E \in \D^b_c(T, \mathbf{Z}_\ell)$, alors $\bRd {\cHom}_{\mathbf{Z}_\ell}(E, \bRd f^!(\mathbf{Z}_\ell)) \in \D^b_c(T, \mathbf{Z}_\ell)$.
\vskip .3cm
{
...
Comme les hypothèses de (II 2.5 $(iii)$) sont réalisées (SGA5 I {\bf 3.3.1}) il résulte du lemme que 
$$
\bRd {\cHom}_{\mathbf{Z}_\ell}(E, \bRd f^! (\mathbf{Z}_\ell)) \in \D^+_c(T, \mathbf{Z}_\ell).
$$
Pour voir qu'il est borné, on peut supposer que $E$ est un $\mathbf{Z}_\ell$-faisceau constructible. Alors, on peut prendre une résolution quasilibre (resp. flasque) de $E$ (resp. $\bRd f^! (\mathbf{Z}_\ell)$) ``canonique'' dans $\mathscr{E}(T, \ell \mathbf{Z}_\ell)$, en ce sens que c'est un système projectif de résolutions quasilibres (resp. flasques) des composants. Comme la dimension quasi-injective des $\bRd f^! (\mathbf{Z}/\ell^{n+1}\mathbf{Z})$ est indépendante de $n$ (preuve de SGA5 I {\bf 3.4.3}), l'assertion en résulte aussitôt. Il reste à voir que, posant
$$
K = \bRd f^! (\mathbf{Z}_\ell)
$$
le morphisme canonique $E \to \D_K \circ \D_K (E)$ est un isomorphisme. Pour cela, désignant par $u_0: \mathbf{Z} \to \mathbf{Z}/\ell \mathbf{Z}$ le morphisme d'anneaux canonique, il suffit (II {\bf 8.2.2}) de voir que le morphisme correspondant
$$
\bLd u^*_0 (E) \to \bLd u^*_0 \D_K \circ \D_K (E)
\leqno{(2.3.5)}
$$
est un isomorphisme. Posant $L = \bLd u^*_0 (K)$, les différentes compatibilités exposées en (I 8) montrent que ({\bf 2.3.5}) s'identifie au morphisme canonique
$$
\bLd u^*_0(E) \to \D_L \circ \D_L(\bLd u^*_0(E)).
$$
Comme $L = \bRd f^! (\mathbf{Z}/\ell \mathbf{Z})$ (1.3.1;2), et $\bLd u^*_0(E) \in \D^b_c(X, \mathbf{Z}/\ell \mathbf{Z})$ (II {\bf 2.13.1}), l'assertion résulte alors de (SGA5 I {\bf 3.4.3}). On aurait pu également utiliser (II {\bf 2.13.9}), qui éclaire bien la situation.
\vskip .3cm
{
...
\vskip .3cm
Preuve. --- L'énoncé est une paraphrase de (SGA5 III {\bf 3.1}). Le fait que $K_X$, $K_Y$ et $K_{X \times_S Y}$ soient dualisants résulte, sur le modèle de la preuve de ({\bf 2.3.3}), de (SGA5 I App.7.5). Utilisant (II {\bf 2.13.9}), et diverses compatibilités évidentes, les assertions a) et b) résultent par simple passage à la limite des assertions correspondantes (SGA5 III {\bf 3.1}) pour les $(\mathbf{Z}/\ell^{n+1}\mathbf{Z})$--Modules $(n \in \mathbf{N})$.

Avant d'énoncer la proposition suivante, précisons quelques définitions et notations de (I {\bf 8.3}). Soit $X$ un schéma noethérien. On définit la catégorie, notée 
$$
\mathbf{Q}_\ell-\fsc(X)
$$
des \emph{$\mathbf{Q}_\ell$-faisceaux} sur $X$, au moyen de la partie multiplicative $\mathbf{Z}_\ell - 0$ de $\mathbf{Z}_\ell$ (I {\bf 8.3}). Étant donné $d \in \mathbf{Z}$ le $\mathbf{Z}_\ell$-faisceau $\mathbf{Z}_\ell(d)$ définit un $\mathbf{Q}_\ell$-faisceau, noté de préférence
$$
\mathbf{Q}_\ell(d).
$$
Comme on l'a indiqué dans (I {\bf 8.3}), on étend sans peine aux $\mathbf{Q}_\ell$-faisceaux le formalisme développé pour les $\mathbf{Z}_\ell$-faisceaux. Ainsi, on dit qu'un $\mathbf{Q}_\ell$-faisceau $F$ est \emph{constructible} (resp. \emph{constant tordu constructible}) s'il est isomorphe à l'image d'un $\mathbf{Z}_\ell$-faisceau du même type. La sous-catégorie pleine, notée $\mathbf{Q}_\ell-\fscn(X)$ (resp. $\mathbf{Q}_\ell-\fsct(X)$), de $\mathbf{Q}_\ell-\fsc(X)$ engendrée par les $\mathbf{Q}_\ell$-faisceaux constructibles (resp. constants tordus constructibles) est \emph{exacte}. De même, si $K$ est un objet de $\D(X, \mathbf{Q}_\ell)$, on dit que $K$ est à cohomologie constructible (resp. constante tordu constructible) s'il est isomorphe dans $\D(X, \mathbf{Q}_\ell)$ à un complexe de $\mathbf{Z}_\ell$-faisceaux constructible (resp. constant tordu constructible). On note
$$
...
\vskip .3cm
Preuve. --- On convient d'identifier, comme on l'a fait dans l'énoncé, un $\mathbf{Q}_\ell$-faisceau constructible ponctuel et le $\mathbf{Q}_\ell$-espace vectoriel de dimension finie correspondant (SGA5 VI {\bf 1.4.3}). Soit
$$
i: x \hookrightarrow X
$$
l'immersion fermée canonique. Posons $K_X = \mathbf{Z}_\ell(d)[2d]$ et $K_x = \mathbf{Z}_\ell$; ce sont des complexes dualisants pour $X$ et $x$ respectivement ({\bf 2.3.3}) et l'on a ({\bf 1.2.3})
$$
\bRd i^!(K_X) \isomlong K_x \quad (\text{canoniquement}).
$$
Notant $\D_X = \D_{K_X}$, $\D_x = \D_{K_x}$, la formule d'induction complémentaire fournit un isomorphisme ({\bf 2.3.2} b) $(ii)$)
$$
...
\vskip .3cm
Remarque {\bf 2.3.8}. --- Nous avons seulement donné ici la variante la moins technique du théorème de dualité locale, et renvoyons le lecteur à (SGA5 I 4) pour des énoncés plus généraux.
\vskip .3cm
{\bf 2.4}. Repla\c{c}ons-nous sous les hypothèses préliminaires de ({\bf 2.3.6}). Nous allons indiquer brièvement comment les constructions de (SGA5 III 3) se transposent dans notre cadre et permettent de démontrer un théorème de \emph{Lefschetz-Verdier}.
\vskip .3cm
{
...
\vskip .3cm
Preuve. --- Formellement identique à celle de (SGA5 III {\bf 3.2}), à partir de ({\bf 2.3.2}) et ({\bf 2.3.6}). On notera que, comme tous les complexes entrant en jeu sont à cohomologie constructible, on dispose sans restriction de l'isomorphisme de Cartan (II {\bf 2.6}).
\vskip .3cm
{
...
\vskip .3cm
Preuve. --- La preuve de l'assertion $(i)$ est formellement identique à celle de (SGA5 III {\bf 2.2.1}). On en déduit $(ii)$ en appliquant aux deux membres le foncteur $\Hom_{\mathbf{Z}_\ell}(\mathbf{Z}_\ell, .)$ (I {\bf 7.4.6}).

Supposons maintenant que $X$ et $Y$ soient \emph{propres} sur $S$, et soient $F \in \D^b_c(X, \mathbf{Z}_\ell)_{\operatorname{torf}}$, $G \in \D^b_c(Y, \mathbf{Z}_\ell)_{\operatorname{torf}}$. Les complexes $\bRd f_* (F)$ et $\bRd g_* (G)$ sont \emph{parfaits}~: en effet, ({\bf 1.1.11} $(i)$ et $(ii)$), ils appartiennent à $\D^b_c (S, \mathbf{Z}_\ell)_{\operatorname{torf}}$ et, comme le corps $k$ est séparablement clos, $\D_c(S, \mathbf{Z}_\ell) = \D_t(S, \mathbf{Z}_\ell)$.
...
\]
Compte tenu de l'isomorphisme ({\bf 2.4.2} $(ii)$), la flèche (a) n'est autre que la restriction du support. La flèche (b) résulte sans peine de l'accouplement ({\bf 2.4.1} b)). La flèche (c) est le cup-produit défini en (II {\bf 2.11}). Enfin, la flèche s'obtient immédiatement à partir du morphisme trace
$$
...
\vskip .3cm
Preuve. --- Utilisant les notations de (II {\bf 2.13.9}), il suffit de voir que pour tout $n \in \mathbf{N}$, le diagramme déduit de ({\bf 2.4.3}) après application du foncteur $\bLd (\alpha_n)^*$ est commutatif. Comme le foncteur $\bLd (\alpha_n)^*$ commute à toutes les opérations usuelles, l'assertion résulte donc de (SGA 5 III {\bf 3.3}) pour les $(\mathbf{Z}/\ell^{n+1}\mathbf{Z})$--Modules $(n \in \mathbf{N})$.
